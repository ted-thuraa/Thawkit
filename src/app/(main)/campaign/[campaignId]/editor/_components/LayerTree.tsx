...
const LayerRow = React.memo(function LayerRow({
  ...
  const appliedComponent = node.layer.componentId
    ? getComponentById(node.layer.componentId)
    : null;
    ...
    const handleSublayerClick = () => {
      if (node.sublayer!.kind === "content") {
        ...
        selectLayerWithSublayer(node.layer.id, {
          ...
          sublayerIndex:
            parentBlockIdx !== undefined
              ? parseInt(parentBlockIdx, 10)
              : node.index,
              ...
              {isRenaming ? (
                <Input
                  ...
                  placeholder={getLayerDisplayLabel(
                    { ...node.layer, customName: undefined },
                    {
                ...
                {interactionTriggerLayerIds.includes(node.id) && (
                  <Icon
                    ...
                    className={cn(
                      ...
                      activeInteractionTriggerLayerId === node.id
                        ? "text-white/80"
                        : "text-white/40",
                    )}
                  />
                ...
                {interactionTargetLayerIds.includes(node.id) &&
                  !interactionTriggerLayerIds.includes(node.id) && (
                    <Icon
                      ...
                      className={cn(
                        "size-3 mr-1 shrink-0",
                        activeInteractionTargetLayerIds.includes(node.id)
                          ? "text-white/70"
                          : "text-white/40",
                      )}
                    />
...
export default function LayersTree({
  ...
  const selectLayerWithSublayer = useEditorStore(
    (s) => s.selectLayerWithSublayer,
  );
  const interactionTriggerLayerIds = useEditorStore(
    (s) => s.interactionTriggerLayerIds,
  );
  const interactionTargetLayerIds = useEditorStore(
    (s) => s.interactionTargetLayerIds,
  );
  const activeInteractionTriggerLayerId = useEditorStore(
    (s) => s.activeInteractionTriggerLayerId,
  );
  const activeInteractionTargetLayerIds = useEditorStore(
    (s) => s.activeInteractionTargetLayerIds,
  );
      ...
      if (editingComponentId) {
        ...
        if (targetVariantId && variantDrafts) {
          ...
          updateComponentDraft(
            editingComponentId,
            targetVariantId,
            updateLayerProps(compLayers, id, { customName: value }),
          );
        }
      } else {
      ...
      if (editingComponentId) {
        ...
        if (targetVariantId && variantDrafts) {
          ...
          updateComponentDraft(
            editingComponentId,
            targetVariantId,
            updateLayerProps(compLayers, id, updates),
          );
        }
      } else {
  ...
  const nodeSelectionData = useMemo(() => {
    ...
    const hasContentSublayerActive =
      storeActiveSublayerIndex !== null && selectedLayerId !== null;
    const hasStyleSublayerActive =
      storeActiveTextStyleKey !== null && selectedLayerId !== null;
    const hasSublayerActive =
      hasContentSublayerActive || hasStyleSublayerActive;
    let activeSublayerNodeId: string | null = null;

    if (
      hasContentSublayerActive &&
      storeActiveTextStyleKey === "listItem" &&
      storeActiveListItemIndex !== null
    ) {
      const listItemMatch = flattenedNodes.find(
        (n) =>
          n.sublayer &&
          n.sublayer.kind === "listItem" &&
          n.sublayer.itemIndex === storeActiveListItemIndex &&
      ...
      const contentMatch = flattenedNodes.find(
        (n) =>
          n.sublayer &&
          n.sublayer.kind === "content" &&
          n.parentId === selectedLayerId &&
          n.index === storeActiveSublayerIndex,
      );
    ...
    if (!activeSublayerNodeId && hasStyleSublayerActive) {
      activeSublayerNodeId =
        flattenedNodes.find(
          (n) =>
            n.sublayer &&
            n.sublayer.kind === "style" &&
            n.sublayer.styleKey === storeActiveTextStyleKey &&
            n.layer.id === selectedLayerId,
        )?.id ?? null;
    }
    ...
    selectedLayerId,
    collapsedIds,
    storeActiveSublayerIndex,
    storeActiveTextStyleKey,
    storeActiveListItemIndex,
  ]);
    ...
    <LayerTreeStoreContext.Provider value={layerTreeStoreValues}>
      ...
      >
        ...
        {activeNode && (
          ...
          >
            {(() => {
              const draggedComponent = activeNode.layer.componentId
                ? getComponentById(activeNode.layer.componentId)
                : null;
            ...
            <span>
              {getLayerDisplayLabel(
                activeNode.layer,
                {
                  component_name: activeNode.layer.componentId
                    ? getComponentById(activeNode.layer.componentId)?.name
                    : null,
