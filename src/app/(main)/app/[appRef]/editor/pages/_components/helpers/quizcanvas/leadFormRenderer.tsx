...
export default function LeadFormItem({ section }: Props) {
  const {
    livemode,
    activeElementId,
    projectData,
  ...
  return (
    // Outer container to simulate the environment background from the image
    <div className="w-full flex   font-inter">
      {/* Main Card Container */}
      <div className="w-full  p-6 sm:p-10 rounded-[32px]  transition-all duration-300">
        {/* Header Section (Icon + Title) */}
        <header className="flex flex-col items-center justify-center pb-8 mb-8  relative ">
          {/* Subtle Grid Background Effect - limited to header area */}

          {/* Icon with Glow Effect */}
          <div className="relative z-10 p-3  rounded-full shadow-lg ">
            <Mail
          ...
          <div
            className={cn(
              "relative mt-4 rounded-md transition-all duration-200",
              isEditorMode && "cursor-text p-1"
            )}
            onClick={(e) => {
              e.stopPropagation();
              if (!isEditorMode || isEditing) return;
              setActiveElementId(titleId);
            }}
            onMouseEnter={() => isEditorMode && setIsHoveredId(titleId)}
            onMouseLeave={() => isEditorMode && setIsHoveredId(null)}
          >
            <div
              className={cn(
            ...
            {isEditing ? (
              <AdvancedTextEditor
                initialHtml={headerTitle as string}
                onUpdate={handleTitleChange}
                className=" text-3xl font-semibold tracking-tight "
              />
            ) : (
              <HtmlParser
                html={headerTitle as string}
                isLive={false}
                editorContentClasses=" text-3xl font-semibold tracking-tight"
              />
            )}
          </div>
        </header>

        {/* Form Body - Spacing and Structure */}
        <Form {...form}>
