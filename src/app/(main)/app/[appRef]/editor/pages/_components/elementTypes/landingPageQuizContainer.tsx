...
const LandingPageQuizComponent = ({ section }: Props) => {
  const {
    pageType,
    livemode,
    questions,
  ...
  useEffect(() => {
    if (selectedQuestion === null && settings?.quizId) {
      const questionId = settings.quizId;
  ...
  return (
    <div
      ref={componentRef}
      className={cn(
        "relative themed-card max-w-lg mx-auto w-full flex items-center justify-center cursor-pointer p-6 md:p-8 min-h-[400px] ",
        // "outline outline-1",
        "focus:outline-none",
    ...
    >
      ...
      <div className="mx-auto w-full h-full">
        {selectedQuestion !== null && (
          <>
            <RenderQuestion element={section as ElementNode} />
          </>
        )}
      </div>
    </div>
...
export default React.memo(LandingPageQuizComponent);
...
const RenderQuestion: React.FC<{ element: ElementNode | null }> = ({
  ...
  return (
    <div className="relative h-full flex flex-col gap-[20px]  lg:gap-[72px]">
      <div>
      ...
      {/* <div>
                <div className="relative w-[520px] h-[450px] overflow-hidden rounded-sm">
                  <Image
    ...
    const updatedOptions =
      ...
      selectedQuestion.options.map((opt) =>
        opt.id === optionId
          ? {
              ...opt,
              media: mediaType as "image" | "emoji" | null | undefined,
              mediaSrc: mediaSrc,
            }
          : opt
      );
  ...
  const handleAddOption = () => {
    ...
    updateQuestion(selectedQuestion.id, {
      options: selectedQuestion?.options
        ? ([...selectedQuestion?.options, newOption] as QuestionOption[])
        : [newOption],
    });
  };
