 ...
 */
export function componentVersionEntityId(
  componentId: string,
 ...
 */
export function markUndoRedoSave(
  entityType: VersionEntityType,
  entityId: string,
): void {
 ...
 */
export function clearUndoRedoSave(
  entityType: VersionEntityType,
  entityId: string,
): void {
  const key = `${entityType}:${entityId}`;
...
export async function recordVersionViaApi(
  ...
  const previousStateForPatch =
    isLayerEntity && Array.isArray(previousState)
      ? stripUIProperties(previousState as Layer[])
      : previousState;
  const currentStateForPatch =
  ...
  const uiMetadata = captureUIMetadata(
    entityType,
    entityId,
    redoPatch as any,
    currentState,
  ...
  const autoDetectedComponents = isLayerEntity
    ? extractComponentRequirements(
        previousState as Layer[],
        currentState as Layer[],
      )
    : [];
  const autoDetectedLayerStyles = isLayerEntity
    ? extractLayerStyleRequirements(
        previousState as Layer[],
        currentState as Layer[],
      )
    : [];
  ...
  if (additionalMetadata) {
    ...
    if (additionalMetadata.requirements?.component_ids) {
      ...
      const additionalComponents =
        additionalMetadata.requirements.component_ids;

      // Combine and deduplicate component IDs
      metadata.requirements = {
        ...(metadata.requirements || {}),
        component_ids: [
          ...new Set([...existingComponents, ...additionalComponents]),
        ],
      };
    }
...
function getAppropriateLayerSelection(
  ...
  for (const layerId of affectedIds) {
    if (
      layerExistsInState(currentState, layerId) &&
      !prioritizedIds.includes(layerId)
    ) {
 ...
 */
function extractComponentRequirements(
  previousState: Layer[],
  currentState: Layer[],
): string[] {
 ...
 */
function extractLayerStyleRequirements(
  previousState: Layer[],
  currentState: Layer[],
): string[] {
  ...
  try {
    ...
    if (entityType === "page_layers" || entityType === "component") {
      ...
      if (patch && currentState) {
        ...
        if (
          lastSelectedLayerId &&
          lastSelectedLayerId !== currentSelectedLayerId
        ) {
