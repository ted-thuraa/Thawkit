...
export function buildLocalizedPageUrls(
  ...
  for (const locale of locales) {
    ...
    const translations = isDefaultLocale
      ? undefined
      : translationsByLocale[locale.id];
    ...
    if (dynamicSlug) {
      ...
      urls[locale.id] = buildLocalizedDynamicPageUrl(
        page,
        allFolders,
        translatedSlug,
        localeArg,
      ...
      urls[locale.id] = buildLocalizedSlugPath(
        page,
        allFolders,
        "page",
        localeArg,
...
export function detectLocaleFromPath(
  ...
  const isValidLocale = validLocaleCodes.some(
    (code) => code.toLowerCase() === firstSegment,
  );
...
export function matchPageWithTranslatedSlugs(
  ...
  const constructedPath =
    "/" + slugParts.map(normalizeSlugSegment).filter(Boolean).join("/");
...
export function folderHasIndexPage(
  ...
  return pages.some(
    (p) =>
      p.id !== excludePageId && p.is_index && p.page_folder_id === folderId,
  );
}
...
export function buildPageTree(
  ...
  const sortChildren = (nodes: PageTreeNode[]) => {
    nodes.sort((a, b) => {
      ...
      const orderA =
        a.type === "folder"
          ? (a.data as PageFolder).order
          : (a.data as Page).order;
      const orderB =
        b.type === "folder"
          ? (b.data as PageFolder).order
          : (b.data as Page).order;
...
export function generateUniqueFolderSlug(
  ...
  // Check if base slug is available (not duplicate and not reserved at root)
  const isBaseSlugReserved =
    parentFolderId === null && isReservedRootSlug(baseSlug);
  if (!existingSlugs.includes(baseSlug) && !isBaseSlugReserved) {
    return baseSlug;
  }
...
export function findNextSelection(
  ...
  const siblingFolders = folders.filter(
    (f) =>
      f.id !== deletedId && f.page_folder_id === parentId && f.depth === depth,
  );
...
export function validateRootIndexPageRequirement(
  ...
  if (currentPage.page_folder_id === null || newFolderId === null) {
    ...
    const otherRootIndexPages = pages.filter(
      (p) => p.id !== currentPage.id && p.is_index && p.page_folder_id === null,
    );
    ...
    if (otherRootIndexPages.length === 0) {
      return {
        isValid: false,
        error:
          "The root folder must have an index page. Please set another page as index first.",
      };
    }
  }
