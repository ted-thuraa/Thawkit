import { QuestionCategories, ScoreTiers } from "../types/project";
...
export const findScoreTierForScore = (
  ...
  // Ensure tiers are sorted by scoreFrom just in case
  const sortedTiers = [...tiers].sort((a, b) => a.scoreFrom - b.scoreFrom);

  return (
    sortedTiers.find(
      (tier) => score >= tier.scoreFrom && score <= tier.scoreTo
    ) || null
  );
};
...
export const getTierColor = (tier: ScoreTiers | null): string => {
  return tier?.scoreColour || DEFAULT_TIER_COLOR;
};
...
export const getTiersRange = (
  ...
  const min = Math.min(...tiers.map((t) => t.scoreFrom));
  const max = Math.max(...tiers.map((t) => t.scoreTo));
...
export const calculateTierSegmentWidths = (
  ...
  return sortedTiers.map((tier) => {
    const tierRange = tier.scoreTo - tier.scoreFrom;
...
export const generateDynamicScoreInfo = (
  ...
  if (selectedTier) {
    ...
    calculatedPercentage = getRandomInRange(
      selectedTier.scoreFrom,
      selectedTier.scoreTo
    );
  } else {
...
export const generateSampleDataFromTiers = (
  ...
  return tiers.map((tier) => {
    const categoryScores = categoryData.map((category) => ({
      ...category,
      scoreTierId: tier.id,
      score_percentage: getRandomScoreInRange(tier.scoreFrom, tier.scoreTo),
