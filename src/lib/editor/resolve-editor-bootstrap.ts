...
export interface EditorBootstrapContext {
  ...
  funnel: FunnelRow;
  /** Ordered by `pages.order` ascending — the funnel's step sequence. */
  pages: PageRow[];
...
export const resolveEditorBootstrap = cache(
  async (campaignId: string): Promise<EditorBootstrapContext> => {
    ...
    const funnelRow = await db.query.funnel.findFirst({
      where: eq(funnel.campaignId, campaignId),
    ...
    const tasks = {
      pages: db.query.pages.findMany({
        where: eq(pages.funnelId, funnelRow.id),
        orderBy: asc(pages.order),
      }),
      components: db.query.component.findMany({
        where: eq(component.organizationId, campaign.organizationId),
      }),
      layerStyles: db.query.layerStyle.findMany({
        where: eq(layerStyle.organizationId, campaign.organizationId),
      }),
    } as const;
