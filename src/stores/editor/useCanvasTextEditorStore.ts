...
import type { InlineVariable } from "@/types/editor/editorTypes";
import { getVariableLabel } from "@/lib/cms-variables-utils";
import type { CollectionField } from "@/types/editor/editorTypes";
...
export const useCanvasTextEditorStore = create<CanvasTextEditorStore>(
  (set, get) => ({
    // State
    isEditing: false,
    editor: null,
    ...
    requestFinish: () => {
      ...
      const setActiveTextStyleKey =
        useEditorStore.getState().setActiveTextStyleKey;
    ...
    updateActiveMarks: () => {
      ...
      if (editor.isActive("dynamicStyle")) {
        ...
        dynamicStyleKey =
          styleKeys.length > 0 ? styleKeys[styleKeys.length - 1] : null;
      }
    ...
    toggleCustomStyle: () => {
      ...
      if (activeMarks.dynamicStyleKey) {
        ...
        editor
          .chain()
          .focus()
          .setMark("dynamicStyle", { styleKeys: [newKey] })
          .run();
      }
      get().updateActiveMarks();
    },
