...
import {
  ...
  createInversePatch,
  isPatchEmpty,
  doesPatchChangeState,
  generatePatchDescription,
  JsonPatch,
} from "@/lib/version-utils";
import {
  markUndoRedoSave,
  componentVersionEntityId,
} from "@/lib/version-tracking";
...
function restoreUIMetadata(
  ...
  try {
    ...
    if (layerIds && Array.isArray(layerIds) && layerIds.length > 0) {
      ...
      if (
        fallbackLayerIds &&
        Array.isArray(fallbackLayerIds) &&
...
export function useUndoRedo({
  ...
  const versionEntityId = useMemo(() => {
    ...
    if (entityType === "component")
      return componentVersionEntityId(entityId, variantId);
    return entityId;
  ...
  const entityState = useVersionsStore((state) => {
    ...
    return (
      state.entityStates[entityKey as keyof typeof state.entityStates] ?? null
    );
  });
  ...
  const updateComponentDraft = useComponentsStore(
    (state) => state.updateComponentDraft,
  );
  ...
  const resolveComponentVariantId = useCallback(
    (
      componentId: string,
      variantDrafts: Record<string, Layer[]>,
    ): string | null => {
      if (variantId && variantDrafts[variantId]) return variantId;
      const componentEntry = useComponentsStore
        .getState()
  ...
  const getCurrentState = useCallback((): any => {
    ...
    switch (entityType) {
      ...
      case "component": {
        ...
        const targetVariantId = resolveComponentVariantId(
          entityId,
          variantDrafts,
        );
        ...
        case "component": {
          ...
          if (process.env.NODE_ENV === "development") {
            ...
            if (duplicates.length > 0) {
              console.error(
                `❌ [applyState] Component ${entityId} - Applying state with DUPLICATE IDs:`,
                duplicates,
              );
          ...
          const variantDrafts =
            useComponentsStore.getState().componentDrafts[entityId];
  ...
  const initialize = useCallback(async () => {
    ...
    try {
      ...
      const existingState =
        useVersionsStore.getState().entityStates[
          `${entityType}:${versionEntityId}`
        ];
      if (
        existingState &&
        (existingState.undoStack.length > 0 ||
          existingState.redoStack.length > 0)
      ) {
      ...
      if (currentState && entityType === "page_layers") {
        currentHash = generatePageLayersHash({
          layers: currentState,
          generated_css: null,
        });
      }
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "versionSaved",
      handleVersionSaved as EventListener,
    );
    return () => {
  ...
  useEffect(() => {
    ...
    const isLayerEntity =
      entityType === "page_layers" || entityType === "component";
    const prevStateForComparison =
      isLayerEntity && Array.isArray(prevState)
        ? stripUIProperties(prevState as Layer[])
        : prevState;
    const currentStateForComparison =
      isLayerEntity && Array.isArray(currentState)
    ...
    if (currentJSON !== prevJSON) {
      ...
      if (
        ...
        const prevDuplicates = prevLayerIds.filter(
          (id, index) => prevLayerIds.indexOf(id) !== index,
        );
        const currentDuplicates = currentLayerIds.filter(
          (id, index) => currentLayerIds.indexOf(id) !== index,
        );

        if (prevDuplicates.length > 0) {
          console.error(
            `❌ [State Tracking] Component ${entityId} - Previous state has DUPLICATE IDs:`,
            prevDuplicates,
          );
        ...
        if (currentDuplicates.length > 0) {
          console.error(
            `❌ [State Tracking] Component ${entityId} - Current state has DUPLICATE IDs:`,
            currentDuplicates,
          );
          console.error("   Previous layer IDs:", prevLayerIds);
          console.error("   Current layer IDs:", currentLayerIds);
  ...
  const undo = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (cacheKey) {
        ...
        if (undoBuffer && undoBuffer.length > 0) {
          ...
          if (
            ...
            if (duplicates.length > 0) {
              console.error(
                `❌ [Local Undo] Component ${entityId} - Buffer state has DUPLICATE IDs:`,
                duplicates,
              );
              console.error("   Layer IDs in buffer:", layerIds);
              console.error(
                "   This indicates corruption when the state was saved to buffer",
              );
            }
          }
      ...
      if (version.metadata?.requirements?.layer_style_ids) {
        const { useLayerStylesStore } = await import(
          "@/stores/useLayerStylesStore"
        );
      ...
      if (currentState && version.undo) {
        const restoredState = applyPatch(
          currentState,
          version.undo as JsonPatch,
        );
      ...
      if (version.snapshot) {
        ...
        if (version.metadata?.requirements?.layer_style_ids) {
          ...
          await restoreLayerStyles(
            version.metadata.requirements.layer_style_ids,
          );
        }
    ...
    try {
      ...
      if (currentState && version.redo) {
        ...
        if (currentJSON === newJSON) {
          ...
          console.warn(
            "⚠️ Current state hash:",
            version.previous_hash?.substring(0, 8),
          );
          console.warn(
            "⚠️ This might indicate we're applying patches out of order",
          );
          // Don't return false - still update the store to keep stacks in sync
        }
    ...
    async (
      ...
      try {
        ...
        const previousHash = previousState
          ...
          : null;
        const currentHash = generatePageLayersHash({
          layers: currentState,
          generated_css: null,
        });
        ...
        const finalDescription =
          description || generatePatchDescription(redoPatch);
 ...
 */
export function markEntityInitializing(
  entityType: VersionEntityType,
  entityId: string,
): void {
  const cacheKey = `${entityType}:${entityId}`;
 ...
 */
export function clearEntityInitializing(
  entityType: VersionEntityType,
  entityId: string,
): void {
  const cacheKey = `${entityType}:${entityId}`;
